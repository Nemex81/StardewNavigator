...
### `numpad-*`
Stringhe di feedback vocale ed HUD generate ed utilizzate dal sottosistema del tastierino numerico (`NumpadController.cs`, `TileInspector.cs`, `GridMovement.cs`, `NumpadActionCatalog.cs`, `NumpadConfigSession.cs`). Includono sia messaggi di stato che stringhe di lettura tile ed output di coordinate.
