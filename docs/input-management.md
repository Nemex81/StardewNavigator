...
* **Protezione Azioni Critiche**: Il resolver centralizzato `NumpadProfileRegistry` impedisce la disattivazione o la rimozione totale delle 5 azioni critiche obbligatorie per un giocatore non vedente (`GridMoveUp`, `GridMoveDown`, `GridMoveLeft`, `GridMoveRight`, `OpenNavigatorMenu`), applicando a runtime un fallback di emergenza locale sul valore predefinito di fabbrica per quel tasto specifico se l'azione non risulta coperta da nessun altro override.
...
* **Invariante di Validazione Configurazione**: La classe `NumpadConfigSession` non consente di confermare le modifiche se `NumpadProfileRegistry.ValidateOverrides` restituisce `false`. Qualsiasi tentativo di salvare un mapping invalido attiva un feedback vocale di errore (`numpad-err-critical-unmapped`).
